...
#'    \item \code{Kmean} unweighted mean of the estimated \eqn{K}-functions
#'    \item \code{Karray} array of estimates, of dimension \code{rlen} x number of quadrats
#'    \item \code{Ktheo} theoretical values under CSR (Poisson process)
...
#' @author Ute Hahn,  \email{ute@@imf.au.dk}
#' @S3method plot kestlist
#' @export

plot.kestlist <- function(klist,  
                  ...
                  lwdtheo = 1, ltytheo = "dashed")
{
  ...
  Kmean <- apply(klist$Karray[, OK], 1, mean, na.rm=T)  
  rr <- klist$r    
  if (is.null(ylim)) ylim <- c(min(klist$Ktheo, klist$Karray), max(klist$Karray, klist$Ktheo))

  if (!add) plot(rr, Kmean, type = "l", ylim = ylim,
                xlab = klist$xlab, ylab = klist$ylab,
                col = col, lwd = lwd, ...)
  else lines(rr, Kmean, col = col, lwd = lwd, ...)

  # plot Poisson reference curve
  if (!add & lwdtheo>0) lines(rr, klist$Ktheo, lwd = lwdtheo, lty = ltytheo)
  
  if(lwdquad > 0) apply(klist$Karray, 2, function(y) lines(rr, y, col=colquad, lwd=lwdquad, ...))
}  
